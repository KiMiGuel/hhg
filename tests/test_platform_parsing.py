...
class TestExtractPlatformName:
    @pytest.mark.parametrize("title,platform,expected", [
...
class TestSocialMatchesCountAsPersons:
    ...
    @staticmethod
    def _m(title, link):
        return LensMatch(rank=1, title=title, link=link, source="",
                         platform=_platform_of(link), reason="")

    def test_instagram_profile_is_person(self):
        m = self._m("Virat Kohli (@virat.kohli) • Instagram photos and videos",
    ...
    def test_instagram_profile_yields_name(self):
        ...
        name = engine._first_person_name(
            self._m("Virat Kohli (@virat.kohli) • Instagram photos and videos",
                    "https://www.instagram.com/virat.kohli/"))
    ...
    def test_handle_only_instagram_is_not_person(self):
        m = self._m("@cristiano • Instagram photos and videos",
...
class TestCorroboration:
    ...
    def test_profiles_found_per_platform(self):
        engine = self._engine_with_results({
            "instagram.com": {"organic_results": [
                {"link": "https://www.instagram.com/virat.kohli/"}]},
            "youtube.com": {"organic_results": [
                {"link": "https://www.youtube.com/@viratkohli"}]},
        })
    ...
    def test_abstain_skips_corroboration(self):
        engine = self._engine_with_results({})
        abstain = LensMatch(rank=None, title="No confident identification",
                            link="", source="", platform="abstain", reason="")
...
class TestBiometricBands:
    def test_bands_follow_sface_threshold(self):
        ...
        assert biometric_verify.BIOMETRIC_MEDIUM == pytest.approx(0.363)
    ...
    def test_wikipedia_link_produces_photo_url(self, monkeypatch):
        ...
        monkeypatch.setattr(bv, "_wikipedia_photo",
                            lambda link: "https://upload.wikimedia.org/x.jpg")
        urls = bv.candidate_photo_urls(
...
class TestNameTorture:
    ...
    def test_honorific_stripped(self):
        n = self._engine()._first_person_name(
            self._m("Dr. Anthony Fauci - Wikipedia"))
        assert n == "anthony fauci"

    def test_president_honorific_stripped(self):
        n = self._engine()._first_person_name(
            self._m("President Barack Obama | Facebook",
    ...
    def test_rip_headline_rejected(self):
        assert self._engine()._first_person_name(
            self._m("RIP Chadwick Boseman")) == ""

    def test_top10_headline_rejected(self):
        assert self._engine()._first_person_name(
            self._m("TOP 10 Tom Cruise Movies")) == ""

    def test_allcaps_headline_rejected(self):
        assert self._engine()._first_person_name(
            self._m("MUSK'S BLACK EYE")) == ""
    ...
    def test_handles_only_rejected(self):
        assert self._engine()._first_person_name(
            self._m("@handle • Instagram photos and videos")) == ""
...
class TestScoringBoosts:
    ...
    def _m(title, link, reason=""):
        return LensMatch(rank=1, title=title, link=link, source="Web",
                         platform="web", reason=reason)

    def test_cross_engine_boost_fires(self):
        # 2 engines: +10, 3 engines: +20
        m2 = self._m("Satya Nadella - Wikipedia",
                     "https://en.wikipedia.org/wiki/Satya_Nadella",
                     reason="visual match #1 | x_engines=2")
        s2, r2 = _score_visual_match(m2)
        m3 = self._m("Satya Nadella - Wikipedia",
                     "https://en.wikipedia.org/wiki/Satya_Nadella",
    ...
    def test_exact_image_boost_fires(self):
        m = self._m("Virat Kohli - Wikipedia",
    ...
    def test_no_person_signal_still_penalized(self):
        ...
        m = self._m("Some random article", "https://example.com/x",
