...
def _frame_quality(frame, faces) -> dict:
    ...
    """
    if not faces:
        return {"score": 0.0, "brightness": 0.0, "sharpness": 0.0,
                "conf": 0.0, "face_ratio": 0.0}
    ...
    return {"score": score, "brightness": brightness, "sharpness": sharpness,
            "conf": conf, "face_ratio": face_ratio}
...
def capture_from_camera(
    ...
    console.print("[dim]Camera controls: [SPACE] capture best frame · [A] auto-capture · "
    ...
    while True:
        ...
        for i, (x, y, w, h, score) in enumerate(faces):
            ...
            cv2.putText(display, f"Face {i}: {score:.2f}", (x, y - 10),
        ...
        cv2.putText(display, f"best score: {best[0]:.2f}",
        ...
        if (auto_capture and not auto_fired and warmed_up and faces
                and q["score"] >= 0.45 and q["brightness"] >= 40):
            key = ord(" ")
            auto_fired = True
        ...
        if key == 27:  # ESC
            ...
            prop = (cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN) if fullscreen \
